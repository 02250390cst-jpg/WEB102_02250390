...
const fileFilter = (req, file, cb) => {
  ...
  if (allowedMimeTypes.includes(file.mimetype)) {
    ...
    const validationError = new Error('Invalid file type. Only JPEG, PNG and PDF files are allowed.');
...
// Basic route for browser checks
app.get('/', (req, res) => {
  res.status(200).json({
    message: 'File Upload API is running',
    upload_endpoint: '/api/upload',
    uploads_static_path: '/uploads/:filename'
  });
});
