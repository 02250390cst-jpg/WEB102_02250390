const formatResponse = (req, res, next) => {
  ...
  res.json = function(obj) {    // Override res.json method
    ...
    const wantsXmlExplicitly =
      req.query.format === 'xml' ||
      (acceptHeader && acceptHeader.includes('application/xml') && !acceptHeader.includes('text/html') && !acceptHeader.includes('application/json'));

    if (wantsXmlExplicitly) {
